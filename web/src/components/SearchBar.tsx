...
export default function SearchBar({
  ...
  type BasicItem = { id: string; name: string; agent?: string };
  ...
  if (!USE_API && IS_STATIC) {
        ...
        if (!allForSuggestRef.current) {
          try {
            const arr = await fetch('/data/products.generated.json').then(r => r.json() as Promise<Array<{ id?: string; name?: string; agent?: string }>>);
            allForSuggestRef.current = (Array.isArray(arr) ? arr : []).map((p) => ({ id: String(p.id || ''), name: String(p.name || ''), agent: String(p.agent || '') }));
          } catch { allForSuggestRef.current = []; }
        }
        const pool = (allForSuggestRef.current || []).filter(x => x.id && x.name && (!agent || x.agent === agent));
