...
export default function CategoryClient({ slug }: { slug: string }) {
  ...
  // Default Anzahl pro Seite (API default ist 24, max 60)
  const pageSize = 24;
  ...
  useEffect(() => {
    ...
    const fetchPromise = (() => {
      ...
      // Agent als Filter mitsenden, damit die Auswahl respektiert wird
      const p1 = new URLSearchParams({ ...baseParams, agent });
      // Preis-Filter an API anhängen, wenn gesetzt
      if (priceMin != null) p1.set('priceMin', String(priceMin));
      if (priceMax != null) p1.set('priceMax', String(priceMax));
    ...
    fetchPromise
      .then((d) => {
        ...
        if (USE_API) {
          ...
          // (Kategorie, Subkategorie, Suche und Preis sind bereits im API-Filter enthalten)
          // Nur client-exklusive Filter anwenden (Favoriten)
        ...
        // STATIC: alles lokal filtern/sortieren/paginieren
        if (agent) items = items.filter((p) => p.agent === agent);
